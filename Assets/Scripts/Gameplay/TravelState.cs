...
namespace CaravanRoguelite.Gameplay
{
    public class TravelState : IGameState
    {
        ...
        private string BuildTravelForecast(MapNode node)
        {
            ...
            {
                NodeType.Combat => $"Ожидается бой-комбо. Текущий забег: акт {_context.Run.Act}, этаж {_context.Run.Floor}.",
                NodeType.City => "Магазин: купить/удалить/улучшить карты и вылечиться.",
                NodeType.Boss => "Босс с особым правилом. После победы акт усилится, а забег продолжится.",
